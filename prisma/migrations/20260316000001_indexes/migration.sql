...
CREATE INDEX CONCURRENTLY "User_referredById_idx" ON "User"("referredById");

-- Profile: filter profiles by role (admin dashboard, role-based user discovery)
CREATE INDEX CONCURRENTLY "Profile_role_idx" ON "Profile"("role");
...
CREATE INDEX CONCURRENTLY "Profile_invitedById_idx" ON "Profile"("invitedById");

-- Profile: paginated user listing sorted by creation date (admin user management)
CREATE INDEX CONCURRENTLY "Profile_createdAt_idx" ON "Profile"("createdAt" DESC);
